...
# ============================================================
# app.py - AI Grading Service with Ollama + OpenAI
# ============================================================
import json
import os
import re
...
# ---------- OpenAI Caller ----------
def _call_openai(prompt: str, file_paths: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            "openai package is not installed. Run: pip install -U openai"
        )
    ...
    if file_paths:
        for path in file_paths:
            ...
            try:
                print(
                    f"Uploading file to OpenAI: {path}",
                    file=sys.stderr,
                )
                ...
                raise RuntimeError(
                    f"Failed to upload file to OpenAI: {path}\n"
                    f"Error: {e}"
                ) from e

    print(
        f"Sending request to OpenAI model: {OPENAI_MODEL}",
        file=sys.stderr,
    )
    ...
    if DEBUG_RAW_OUTPUT:
        print("=" * 80, file=sys.stderr)
        print("RAW OPENAI OUTPUT:", file=sys.stderr)
        print(raw_output, file=sys.stderr)
        print("=" * 80, file=sys.stderr)

    return _extract_first_json(raw_output)

# ---------- Unified AI Caller (with Ollama fallback) ----------
def _call_ai(prompt: str, file_paths: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        print(
            f"AI provider '{AI_PROVIDER}' failed: {e}. "
...
def grade(req: GradeRequest):
    raw_output_debug = ""
    try:
        # Extract text for prompt (still used for both Ollama and OpenAI)
        max_input = MAX_INPUT_CHARS_FOR_SMALL_MODEL if "4b" in OLLAMA_MODEL.lower() or "3b" in OLLAMA_MODEL.lower() else MAX_EXTRACTED_CHARS
        rubric_text = rubric_content(req, max_input)
        submission_text = submission_content(req, max_input)

        # Collect file paths for provider file inputs
        file_paths = []
        seen_file_paths = set()
