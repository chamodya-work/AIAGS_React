...
export default function ManageUsers() {
  ...
  const filtered = users.filter(u => {
    const searchable = `${u.display_name || ''} ${u.full_name || ''} ${u.email || ''} ${u.student_no || ''} ${u.university_user_id || ''}`.toLowerCase();
        ...
        <div className="filters-section">
          <div className="filter-group">
            ...
            <input
              ...
              placeholder="Name, email, student no, university id..."
              value={search}
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  <tr key={u.user_id}>
                    ...
                    <td>{u.university_user_id || '-'}</td>
                    <td>
                      <span className={`badge ${u.is_active === 0 ? 'badge-danger' : 'badge-success'}`}>
                        {u.is_active === 0 ? 'Inactive' : 'Active'}
                      </span>
                    </td>
                    <td>{u.last_login_at ? new Date(u.last_login_at).toLocaleString() : '-'}</td>
                    <td>{u.student_no || '-'}</td>
                    <td><button className="icon-btn" onClick={() => openEdit(u)}>Edit</button></td>
                    <td><button className="icon-btn" onClick={() => handleDelete(u.user_id)}>Delete</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
