  ...
  const handleView = async () => {
    ...
    try {
      ...
      if (published) {
        ...
        setNotReleased(true); // submitted but not published yet
      } else {
        setNotReleased(true); // no submission found
      }
    } catch (err) { setError(err.message); }
        ...
        {!loading && notReleased && (
          <div style={{ textAlign:'center', padding:'48px 20px', background:'#fff8e1', borderRadius:10, border:'1px solid #ffc107', marginTop:20 }}>
            ...
            <div style={{ fontSize:14, marginTop:8, color:'#666' }}>
              Your grades have not been published for this assignment yet. Please check back later.
            </div>
          </div>
        )}
          ...
          <div style={{ marginTop:24 }}>
            ...
            <div style={{ background:'linear-gradient(135deg, #144573, #1e81af)', borderRadius:12, padding:28, color:'white', marginBottom:20, display:'flex', gap:40, flexWrap:'wrap', alignItems:'center' }}>
              <div style={{ textAlign:'center' }}>
                <div style={{ fontSize:12, opacity:0.8, textTransform:'uppercase' }}>AI Score</div>
                <div style={{ fontSize:42, fontWeight:700 }}>{result.ai_grade ?? '—'}</div>
              </div>
              <div style={{ textAlign:'center' }}>
            ...
            <div style={{ textAlign:'center', marginTop:24 }}>
              <button className="btn btn-secondary" onClick={() => window.print()}>🖨️ Print Result</button>
            </div>
          </div>
        )}
      </div>
